...
class _ReaderMenuState extends State<ReaderMenu> with SingleTickerProviderStateMixin {
  ...
  buildTopView(BuildContext context) {
    return Positioned(
      ...
      right: 0,
      child: Container(
        decoration: BoxDecoration(color: MyColors.paper, boxShadow: Styles.borderShadow),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Text('${percentage.toStringAsFixed(1)}%', style: TextStyle(color: MyColors.lightGray, fontSize: 12)),
        ],
      ),
    );
  }
  ...
  previousArticle() {
    if (this.widget.articleIndex == 0) {
      Toast.show('已经是第一章了', context,duration: Toast.LENGTH_SHORT,gravity: Toast.BOTTOM);
      return;
    }
  ...
  nextArticle() {
    if (this.widget.articleIndex == this.widget.chapters.length - 1) {
      Toast.show('已经是最后一章了', context,duration: Toast.LENGTH_SHORT,gravity: Toast.BOTTOM);
      return;
    }
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          Image.asset(icon),
          SizedBox(height: 5),
          Text(title, style: TextStyle(fontSize: fixedFontSize(12), color: MyColors.darkGray)),
        ],
      ),
    );
  }
